...
/* $Id: dbus.c 312189 2011-06-15 11:35:34Z derick $ */
// ByteArray. add by rock <shirock.tw@gmail.com>
...
{
    unsigned char *binstr, *data;
    int binstr_len;
    //zval *object = getThis();

    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "s", &binstr, &binstr_len) == FAILURE) {
        RETURN_NULL();
    }

    bytearray_object *this = (bytearray_object *)zend_object_store_get_object(getThis() TSRMLS_CC);
    this->data = estrndup(binstr, binstr_len); // estrndup==memcpy, binary safe.
    this->len = binstr_len;
    //fprintf(stderr, "data: %s\n", this->data);
    //fprintf(stderr, "len: %d\n", this->len);
}

PHP_METHOD(Bytearray, __toString)
{
    //zval *object = getThis();
    bytearray_object *this = (bytearray_object *)zend_object_store_get_object(getThis() TSRMLS_CC);
    RETURN_STRINGL(this->data, this->len, 1);  // binary safe.
}

const zend_function_entry bytearray_methods[] = {
    PHP_ME(Bytearray,  __construct,     NULL, ZEND_ACC_PUBLIC | ZEND_ACC_CTOR)
    PHP_ME(Bytearray,  __toString,      NULL, ZEND_ACC_PUBLIC)
    {NULL, NULL, NULL}
};
...
zend_object_value bytearray_create_handler(zend_class_entry *class_type TSRMLS_DC)
{
    zval *tmp;
    zend_object_value retval;

    bytearray_object *intern = (bytearray_object *)emalloc(sizeof(bytearray_object));
    memset(intern, 0, sizeof(bytearray_object));
    intern->std.ce = class_type;
    //fprintf(stderr, "create ByteArray\n");
    ...
    retval.handle = zend_objects_store_put(intern, 
        ...
        bytearray_free_storage, NULL TSRMLS_CC);
    retval.handlers = &bytearray_object_handlers;

    return retval;
}

// indexer get
static zval *bytearray_indexer_read_dimension(zval *object, zval *offset, int type TSRMLS_DC)
{
	bytearray_object *this;
    long idx;
	zval *str;

	this = (bytearray_object *) zend_object_store_get_object(object TSRMLS_CC);
	MAKE_STD_ZVAL(str); // return value

	if (offset->type != IS_LONG) {
		ZVAL_STRINGL(str, this->data, 1, 1);
		return str;
	}

	idx = offset->value.lval;
	if (idx < 0) { // I allow negative.
		idx += this->len;
	}
	//fprintf(stderr, "offset: %ld; len: %ld\n", idx, this->len);
	if (idx >= 0 && idx < this->len) {
		ZVAL_STRINGL(str, this->data + idx, 1, 1);
	}
	else {
		php_error_docref(NULL TSRMLS_CC, E_NOTICE, "Uninitialized string offset: %ld", idx);
		ZVAL_STRINGL(str, this->data + idx, 0, 1);
		//ZVAL_NULL(str);
	}
	return str;
}
...
// indexer set
void bytearray_indexer_write_dimension(zval *object, zval *offset, zval *value TSRMLS_DC)
{
	bytearray_object *this;
    long idx;
	char *vbuf = NULL;
	...
	case IS_STRING:
		if (value->value.str.len > 0)
			this->data[idx] = value->value.str.val[0];
		break;
	case IS_LONG:
		spprintf(&vbuf, 0, "%ld", value->value.lval);
		this->data[idx] = vbuf[0];
		efree(vbuf);
		break;
	default:
		break;
...
// indexer isset
int bytearray_indexer_has_dimension(zval *object, zval *offset, int check_empty TSRMLS_DC)
{
	bytearray_object *this;
    long idx;
	...
	if (offset->type != IS_LONG || idx >= this->len || idx < 0)
		return 0;
	return 1;
}

// count()
int bytearray_count_elements(zval *object, long *count TSRMLS_DC)
{
	bytearray_object *this;
	this = (bytearray_object *) zend_object_store_get_object(object TSRMLS_CC);
	*count = this->len;
...
// next, register in PHP_MINIT_FUNCTION
// end of bytearray.
...
PHP_MINIT_FUNCTION(dbus)
{
	...
	// bytearray, added by rock
	zend_class_entry ce;
	INIT_CLASS_ENTRY(ce, "ByteArray", bytearray_methods);
	//INIT_CLASS_ENTRY(ce, "DbusByteArray", bytearray_methods);
	bytearray_ce = zend_register_internal_class(&ce TSRMLS_CC);
	bytearray_ce->create_object = bytearray_create_handler;
    memcpy(&bytearray_object_handlers, zend_get_std_object_handlers(), sizeof(zend_object_handlers));
    bytearray_object_handlers.clone_obj = NULL;
    bytearray_object_handlers.read_dimension = bytearray_indexer_read_dimension;
    bytearray_object_handlers.write_dimension = bytearray_indexer_write_dimension;
    bytearray_object_handlers.has_dimension = bytearray_indexer_has_dimension;
    bytearray_object_handlers.count_elements = bytearray_count_elements;
    // end of bytearray.
...
// php-dbus supports bytearray, added by rock
static int dbus_append_var_php_bytearray(php_dbus_data_array *data_array, DBusMessageIter *iter, bytearray_object *obj TSRMLS_DC)
{
	DBusMessageIter listiter;
	char type_string[2] = "y";
	zval **entry;
	int i;
	...
	for (i = 0; i < obj->len; ++i) {
		dbus_message_iter_append_basic(&listiter, DBUS_TYPE_BYTE, &obj->data[i]);
	}
...
static int dbus_append_var(zval **val, php_dbus_data_array *data_array, DBusMessageIter *iter, char *type_hint TSRMLS_DC)
{
	...
	switch (Z_TYPE_PP(val)) {
		...
		case IS_OBJECT:
			...
			// php-dbus supports bytearray, added by rock
			if (obj->ce == bytearray_ce) {
				dbus_append_var_php_bytearray(data_array, iter, (bytearray_object*) obj TSRMLS_CC);
			}
			// end bytearray
		...
		case DBUS_TYPE_ARRAY:
			{
				...
				// php-dbus supports bytearray, added by rock
				int ar_type = 0, ar_len = 0, i_ar;
				ar_type = dbus_message_iter_get_arg_type(&subiter);
				if (ar_type == DBUS_TYPE_BYTE) {
					bytearray_object *new_obj;
					const int8_t *stat;
					dbus_message_iter_get_fixed_array(&subiter, &stat, &ar_len);

					dbus_instantiate(bytearray_ce, return_value TSRMLS_CC);
					new_obj = (bytearray_object*) zend_object_store_get_object(return_value TSRMLS_CC);
					new_obj->data = emalloc(ar_len);
					new_obj->len = ar_len;
					memcpy(new_obj->data, stat, ar_len);
					break; // switch
				}
				// end bytearray
