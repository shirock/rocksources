...
class OMXPlayer(dbus.service.Object):
    ...
    def __init__(self, mrl, x, y, width, height, keepratio, autoplay=True, options=None, bus = None, path = None):
        ...
        # mplayer config:
        # sys: /etc/mplayer/mplayer.conf
        # user: ~/.mplayer/config
        cmd.extend(['-noborder', '-quiet', '-ontop',
#            '-vo', 'vdpau', '-vc', 'ffmpeg12vdpau,ffwmv3vdpau,ffvc1vdpau,ffh264vdpau,ffodivxvdpau',
            '-geometry', '%dx%d+%d+%d' % (visual_area[2], visual_area[3], visual_area[0], visual_area[1]), 
            self.mrl])
        debug(cmd)
        try:
...
def detect_video_information(mrl):
    ...
    try:
        ...
        output = subprocess.Popen(["/usr/bin/avconv", "-i", mrl], 
            stdout=subprocess.PIPE,
...
def get_pid_filepath():
    ...
    return "/tmp/omxplayer-dbus-service-session.pid"
...
def log(s):
    ...
    log_filepath = "/var/log/omxplayer-dbus/session.log"
